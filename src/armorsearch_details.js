  ...
  useEffect(() => {
    const fetchArmorDetails = async () => {
      ...
      try {
        ...
        if (data.setName) {
          const setResponse = await fetch(`https://mhw-db.com/armor-sets?name=${data.setName}`);
                  ...
                  <>
                    ...
                    <ul>
                      {armorSetBonus.ranks.map((rank, index) => (
                        <li key={index}>
                          {rank.pieces} pieces: {rank.skill.skillName} (Level {rank.skill.level})
                          <br />
                          <span className="text-sm text-gray-500">{rank.skill.description}</span>
                        </li>
                      ))}
                    </ul>
                  </>
